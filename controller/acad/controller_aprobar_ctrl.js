...
$(document).ready(function () {
  ...
  function consultar_solicitud() {
    //funcion para consultar la solicitud de numeros de control
    var obj;
  ...
  function status_botones() {
    //funcion para cambiar el estatus de los botones si no hay una solicitud o si ya se aprobo
  ...
  function consultar_numeros() {
    //funcion para consultar los numeros de control
  ...
  function crear_numeros() {
    //funcion para generar los numeros de control y mostrarlos en la tabla (esto solo es visual, los numeros reales se generan en php)
    var fecha = new Date();
    var num = $("#num_matriculas").val();
    ultimoNumero++;

    for (var i = 0; i < num; i++) {
      $("#tabla_num_ctrl tbody").append("<tr><th scope=\"row\">".concat(ultimoNumero, "</th><td>").concat(fecha.getDate(), "-").concat(fecha.getMonth() + 1, "-").concat(fecha.getFullYear(), "</td></tr>"));
      ultimoNumero++;
    }
  }

  function enviar_numeros() {
    //esta funcion aprueba la solicitud y envia la cantidad de numeros que necesitamos al modelo
